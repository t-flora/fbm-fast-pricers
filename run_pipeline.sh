...
if [ "$NO_BUILD" -eq 0 ]; then
    echo ""
    echo "── Step 1/9: cmake build ──"
    cmake -B build -DCMAKE_BUILD_TYPE=Release -Wno-dev --log-level=WARNING
    cmake --build build --parallel
    echo "  done."
else
    echo ""
    echo "── Step 1/9: cmake build — SKIPPED (--no-build) ──"
fi

# ── Step 2: C++ benchmark ─────────────────────────────────────────────────────
echo ""
echo "── Step 2/9: C++ benchmark ──"
./build/benchmark

# ── Step 3: Scaling + construction breakdown + memory plots ───────────────────
echo ""
echo "── Step 3/9: plot_scaling.py ──"
uv run python plots/plot_scaling.py

# ── Step 4: Structural analysis ───────────────────────────────────────────────
echo ""
echo "── Step 4/9: plot_structure.py (N-small=${STRUCT_SMALL}, N-large=${STRUCT_LARGE}) ──"
uv run python plots/plot_structure.py --N-small "${STRUCT_SMALL}" --N-large "${STRUCT_LARGE}"

# ── Step 5: MC convergence ────────────────────────────────────────────────────
echo ""
echo "── Step 5/9: validate_convergence.py (seeds=${CONV_SEEDS}, max-M=${CONV_MAX_M}) ──"
uv run python data/validate_convergence.py --n-seeds "${CONV_SEEDS}" --max-M "${CONV_MAX_M}"

# ── Step 6: Stability ─────────────────────────────────────────────────────────
echo ""
echo "── Step 6/9: validate_stability.py ──"
uv run python data/validate_stability.py

# ── Step 7: Lévy benchmark + roughness premium ────────────────────────────────
echo ""
echo "── Step 7/9: validate_asian.py (M=${M_ASIAN}, N=${N_ASIAN}) ──"
uv run python data/validate_asian.py --M "${M_ASIAN}" --N "${N_ASIAN}"

# ── Step 8: IV smile vs SPY (requires internet) ───────────────────────────────
echo ""
if [ "$NO_IV" -eq 0 ]; then
    echo "── Step 8/9: validate_iv.py (M=${M_IV}, N=${N_IV}) ──"
    uv run python data/validate_iv.py --M "${M_IV}" --N "${N_IV}" || {
        echo "  WARNING: validate_iv.py failed (network or data issue) — continuing."
    }
else
    echo "── Step 8/9: validate_iv.py — SKIPPED (--no-iv) ──"
fi

# ── Step 9: Sensitivity heatmap + price vs strike ─────────────────────────────
echo ""
echo "── Step 9/9: plot_sensitivity.py (M=${M_SENS}, N=${N_SENS}) ──"
