...
def plot_surface(H_grid, nu_grid, price_grid, out_path):
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))

    # ── Panel 1: Heatmap price vs (H, nu) ───────────────────────────────────
    ax = axes[0]
    im = ax.imshow(price_grid, aspect="auto", origin="lower",
                   extent=[nu_grid[0], nu_grid[-1], H_grid[0], H_grid[-1]],
                   cmap="RdYlGn_r")
    cbar = fig.colorbar(im, ax=ax)
    cbar.set_label("Asian call price")

    # Annotate cells
    for i in range(len(H_grid)):
        for j in range(len(nu_grid)):
            ax.text(nu_grid[j], H_grid[i], f"{price_grid[i, j]:.1f}",
                    ha="center", va="center", fontsize=8, color="black")

    ax.set_xlabel("Vol-of-vol nu")
    ax.set_ylabel("Hurst exponent H")
    ax.set_title("ATM Asian Call Price vs (H, nu)\n(K=S0=100, T=1yr)")
    ...
    for nu, color in zip(nu_grid, colors):
        nu_idx = list(nu_grid).index(nu)
        ax2.plot(H_grid, price_grid[:, nu_idx], marker="o", linewidth=2,
                 markersize=6, color=color, label=f"nu={nu:.2f}")
...
def plot_strike_sensitivity(H_grid, K_grid, results, nu_fixed, out_path):
    ...
    colors = [cmap(x) for x in np.linspace(0.05, 0.95, len(H_grid))]
    ...
    ax.axvline(S0, color="gray", linestyle=":", linewidth=1.2, alpha=0.7,
               label=f"ATM (K=S0={S0:.0f})")
    ax.set_xlabel("Strike K")
    ax.set_ylabel("Asian call price")
    ax.set_title(f"Asian Call Price vs Strike for Varying H\n(nu={nu_fixed:.2f}, S0={S0:.0f}, T={T:.0f}yr)")
...
def main():
    ...
    parser.add_argument("--M", type=int, default=3000,
                        help="Monte Carlo paths per grid point (default: 3000)")
    ...
    plot_surface(H_GRID, NU_GRID, price_grid,
                 os.path.join(out_dir, "sensitivity_surface.png"))
